...
当前 OAuth 流程可能不会写入 `Modelserver.ProjectID` 和 `Agentserver.WorkspaceID`。控制台需要做补全：

- modelserver：使用保存的 `modelserver_api_key` 调用 `GET https://codeapi.cs.ac.cn/api/v1/projects`，优先选择第一个 active project；如果本地已有 `ProjectID`，优先匹配它。选中后写回 state。
- agentserver：使用保存的 `agentserver_ws_api_key` 调用 `GET https://agent.cs.ac.cn/api/workspaces`，优先选择第一个 workspace；如果本地已有 `WorkspaceID`，优先匹配它。选中后写回 state。

如果远端 API 因 token 类型或权限返回 401/403，页面显示“需要重新登录”或“无法读取项目/工作空间”，并提供重新配置入口。
...
- 控制台状态聚合：
  ...
  - 缺失 project/workspace 时能自动从列表补全并写回 state。
...
- modelserver OAuth token 可能不能访问项目列表或订阅 API：页面显示需要重新登录或权限不足；不影响启动 Codex/VS Code。
- agentserver device token 可能不能访问 `/api/workspaces`：页面显示工作空间不可用；不影响右键打开当前前端。
