# Codex Desktop 默认安装模式

**Date**: 2026-06-07
**Status**: Design approved; written spec pending user review
**Scope**: Windows x64 安装包新增 Codex Desktop 离线安装与向导模式切换。默认安装
...
官方 Codex 文档确认 Codex app、CLI、IDE extension 共用配置层，用户级配置位于
`~/.codex/config.toml`，Codex Desktop 也注册 `codex://` URL scheme。因此默认模式
可以复用现有 `internal/codex/config.go` 写入 modelserver provider 配置，并通过
深链打开 Codex Desktop。
...
|---|---:|---|---|---|
| Codex Desktop | 是 | Codex Desktop 离线包、launcher、onboarding、codex 配置工具 | 安装和配置 Codex Desktop | 打开 Codex Desktop 新会话，优先携带文件夹路径 |
...
- 未选中：运行 Codex Desktop 安装脚本并写入 `codex_desktop` 模式
...
- `codex-desktop-installer`：从用户提供的官方地址
...
构建脚本需要记录 Codex Desktop 离线包的文件名、大小和 SHA256。安装时优先验证
本地 payload；如果 payload 缺失或校验失败，给出明确错误，不默默回退到 VS Code。
...
- `install.go`：安装 Codex Desktop 离线 payload。
- `launch.go`：打开 `codex://threads/new` 深链。
- `packaging/windows/codex-desktop-manifest.json`：记录下载 URL、大小、SHA256、安装参数。
...
构建脚本新增 Codex Desktop payload 缓存步骤：

1. 下载用户提供的官方 URL。
2. 解析最终文件名和扩展名。
3. 计算 SHA256 和文件大小。
4. 写入 `packaging/windows/codex-desktop-manifest.json`。
5. 把 payload 复制到 `dist/cache/codex-desktop/`。
6. Inno Setup 将 payload 打入安装包。
...
- Codex Desktop payload 缺失：安装失败并提示离线包缺失。
- SHA256 不匹配：安装失败并提示离线包校验失败。
- Codex Desktop 安装程序退出非零：显示安装错误，不切换到 VS Code。
...
- `EnsureFrontend` 在 `codex_desktop` 下调用 Codex Desktop installer，不调用 VS Code。
...
Codex Desktop 官方下载 URL 可能返回 MSIX/App Installer 或 EXE。实现前必须先下载一次，
确认文件类型和静默安装方式：

- EXE：优先查找 `/quiet`、`/silent` 或官方安装参数。
- MSIX/App Installer：优先使用 PowerShell `Add-AppxPackage` 或 App Installer 支持的
  静默路径。
- Microsoft Store package：如果离线 payload 无法静默安装，需要在安装脚本里给出清晰
  错误，并保留 manifest 校验。

无论是哪种文件类型，默认用户体验仍是一个安装包内置 Codex Desktop 离线包，不要求用户
在安装过程中联网下载 Codex Desktop。
